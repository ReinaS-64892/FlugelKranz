...
4. グリップを離すと、その時点の移動速度・全軸角速度を引き継いで慣性移動します。「オフにする」は入力と慣性を止め、位置・姿勢を保持します。「接続時の位置・姿勢に戻す」はオフにして初期オフセットへ復元します。

通常のウィンドウ終了時も復元します。強制終了やサービス切断では復元できない場合があります。他のツールがオフセットを変更した場合は停止し、その変更を上書きしません。トラッキングを失った手は操作と該当する慣性を解除し、復帰後はグリップを離すまで再開しません。

## 慣性パラメーター

現段階の仮 UI では、次の値を実行中にも変更できます。

- **ステップモード**はグリップ解放時の慣性を無効にします。スムーズ処理は引き続き適用されます。
- **慣性カットオフ**は解放速度が Drag `5 cm/s`、Turn `5 °/s` 未満なら慣性を開始せず、減速中に閾値を下回った場合も停止します。チェックを外すと無効になります。
- **慣性加速倍率**は解放速度を Drag・Turn 別に `0.0`～`5.0` 倍します。
- **慣性減速**は毎秒の指数減衰率です。`0.0` は減速なし、`10.0` は急速な減速で、既定値は `0.01 /秒` です。
- **慣性減速免除**の時間は、現在速度からカットオフまで通常減速する予測時間に対する割合です。免除割合 `1.0` なら、その期間は完全に減速しません。
- **ドラグスムーズ**は Drag と Turn の追従を `0.0`～`1.0` 秒の時定数で平滑化します。
- **ドラブレーキ値**は新しくグリップした瞬間に既存の線速度・角速度を消す割合です。`0.0` は維持、`1.0` は全停止です。
- **方向補正**は短時間かつ直線的な操作だけを対象に、解放速度の方向を軌跡全体へ近づけます。Drag・Turn ごとに最大時間と反映強度を設定できます。
