...
- **Z加速**はプレイヤーの正面を `Z+` として、Drag 解放時の前後成分だけを追加倍率で強めます。方向との内積の絶対値を重みとするため、横方向には追加加速をかけません。範囲は `1.0`～`5.0`、既定値は `1.5` です。
- **慣性加速ブーストモード**を有効にすると、Drag 中のコントローラー移動速度がカットオフを超えている間は Drag ブレーキをかけず、解放時の加速を残っている慣性へ加算します。コントローラーが再び低速または停止するとブレーキも再開します。加算後の速度は、残っている慣性と今回の解放加速の大きい方を基準に最大倍率で制限され、既定値は有効・`1.5` 倍（`1.0`～`4.0`）です。
...
- **ドラグブレーキ**は新しくグリップした瞬間に対応する慣性を完全に減衰させます。左手の Drag は線速度だけ、右手の Turn は角速度だけに作用します。適用時間を `0.0`～`1.0` 秒で設定でき、`0.0` 秒なら即時停止します。発動時は対応する慣性減速免除を解除します。
...
自動テストは三軸回転、左手の固定点、左右同時操作、慣性・減速・カットオフ・ブレーキ、既存オフセット、入力への変換の混入防止、トラッキング喪失、オン・オフ・復元、UI のバインディングと接続エラーを対象にします。実機では VRChat を動かしたまま、左右個別・同時操作、各回転軸、グリップ解放、慣性パラメーター変更、オフ、復元、終了を確認してください。自動テストだけではランタイム・コントローラー・VRChat の組み合わせを保証できません。
